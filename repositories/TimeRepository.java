...
public class TimeRepository {
    ...
    private static final List<Time> times = new ArrayList<>() {
        {
            add(new Time(proximaChaveUnica,
                    "Sapecas do morro",
                    PessoaRepository.obterPorNome("Lucas").stream().map(pessoa -> (Jogador)pessoa).toList(),
    ...
    public static List<Time> obter(String nome) {
        return times.stream().filter(time -> time.getNome().toUpperCase().contains(nome.toUpperCase())).toList();
    }
